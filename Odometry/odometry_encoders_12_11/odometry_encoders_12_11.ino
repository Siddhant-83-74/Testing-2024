...
IntervalTimer theEncoders;

float acc_x,acc_y,acc_z,dis_acc_x,dis_acc_y,dis_acc_z;
...
void setup() {
  ...
  theTimer.priority(0);
  theEncoders.priority(1);
  theTimer.begin(calc_rpm, 75000);
  theEncoders.begin(enc_dis,75000);
...
int x_enc_dis=0,y_enc_dis=0;
void enc_dis()
{
x_enc_dis=-m[0].read()-m[1].read()+m[2].read()+m[3].read();
y_enc_dis=-m[0].read()+m[1].read()+m[2].read()-m[3].read();

Serial.printf("X_distance enc: %d   ",x_enc_dis);
Serial.printf("Y distance enc: %d   ",y_enc_dis);
//acc_value();
}
...
void acc_value(void)
{
  ...
  dis_acc_y+=acc_y*0.075*0.075;
  dis_acc_x+=acc_x*0.075*0.075;
  dis_acc_z+=acc_z*0.075*0.075;

// Serial.printf("Y distance acc: %d   ",dis_acc_y);
// Serial.printf("X distance acc: %d   ",dis_acc_x);
//delay(1000);
