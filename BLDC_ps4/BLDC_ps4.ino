...
if(left_y>=0)
    val=abs(map(left_y,127,1,30,0));
else
    val=abs(map(left_y,0,-128,25,93));
    ...
    /*At generic callibration at medium outrunner 750g
     * 285mV   
     * 292mV : 176rpm   
     * 295   : 237
     * 300   : 330
     ...
     * 353:  1420
     * 
     * At skateboard callibration at small outrunner 200g
     * 278    --- 30
     * 290: 250 ---- 40
     * 295: 357 ---- 45
     * 300 : 456 --- 49
    ...
    */
    Serial.println(val);
    //Serial.println(map(val,-128,127,0,180));

    if(buttons==32 && flag==0)
    {      Serial.println("button is on");

      flag=1;}
      
     // delay(100);
    if(buttons==16 && flag==1)
    {    Serial.println("button is off,joystick activated");

      flag=0;}

      
    if( flag==1){
    s1.write(map(val,-128,127,0,180));
    delay
    //flag=0;
