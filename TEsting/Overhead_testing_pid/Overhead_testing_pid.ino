...
volatile long last_Time[]={0,0,0,0};
void setup() {
...
void calc_rpm() {
 ...
 // count=newPosition<oldPosition?-count:count;
  ...
  Serial.printf("RPM_ISR: %ld ",(micros()-last_Time[0]));
...
myusb.Task();
  ...
  }  Serial.printf("PS4_ISR: %ld ",(micros()-last_Time[1]));

}
volatile float kp=30.0;
volatile float ki=145.0;
...
drive(pwm_pid, m2_pwm, m2_dir); 

  lastError=error;
    Serial.printf("PID_ISR: %ld \n",(micros()-last_Time[2]));
